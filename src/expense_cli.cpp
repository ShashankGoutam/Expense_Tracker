...
int tracker::handle_command(int argc , char **argv){
    if(argc==1){
        std::cerr << "No command provided. Use help for usage information" << std::end;
        return 1;
    }
    const std::string cmd = argv[1];
    auto it = command_handles.find(cmd);
    if(it == command_handles.end()){
        std::cerr << "Unknown command" << cmd << "Use 'help' for proper usage information" << std :: endl;
        return 1;
    }
